...
const Attendence = require("../models/Utils/attendence");
...
exports.adminCreate = catchAsyncError(async (req, res, next) => {
  const { userName, password, email, name, code } = req.body;
  if (!userName | !password) {
    return next(new ErrorHandler("Please enter username & password", 400));
  }
  if (!email | !name) {
...
exports.getSingleManager = catchAsyncError(async (req, res, next) => {
  const employee = await Manager.findById(req.params.id).populate(
    "activeProject"
  );
  if (!employee) {
...
/* =================================================================
        Hr Update Password (/api/v1/subadmin/password/update) (req : Put)
   ==================================================================== */
exports.updatePassword = catchAsyncError(async (req, res, next) => {
  const { oldPassword, newPassword } = req.body;
  const user = await Subadmin.findById(req.user.id).select("+password");

  const isPassowrdMatched = await user.comparePassword(oldPassword);
  ...
  sendToken(user, "Hr", 200, res);
});

/* =================================================================
        Hr Update Profile (/api/v1/subadmin/profile/update) (req : Put)
   ==================================================================== */
exports.updateProfile = catchAsyncError(async (req, res, next) => {
  ...
  const user = await Subadmin.findByIdAndUpdate(req.user.id, newUserData, {
    new: true,
    runValidators: true,
    useFindAndModify: false,
  });
...
/* =================================================================
        Hr Update Avatar (/api/v1/subadmin/avatar/update) (req : Put)
   ==================================================================== */
exports.updateAvatar = catchAsyncError(async (req, res, next) => {
  const newUserData = {};

  if (req.body.avatar !== "") {
    const user = await Subadmin.findById(req.user.id);
...
/* ===================================================
        Resign Manager (/api/v1/manager/resign/:id) (req : put)
   =================================================== */
exports.resignManager = catchAsyncError(async (req, res, next) => {
  ...
  if (!manager) {
    return next(new ErrorHandler("Manager not found", 400));
  }
  if (manager) {
    const userData = {
      manager: null,
    ...
    await Project.findByIdAndUpdate(project._id, userData, {
      new: true,
      runValidators: true,
      useFindAndModify: false,
    });
    await Manager.findByIdAndUpdate(manager._id, data, {
      new: true,
      runValidators: true,
      useFindAndModify: false,
    });
  ...
  res.status(200).json({
    success: true,
    message: "Resign Manager",
    project: newProject,
  });
});

/* ===================================================
        Assign Manager (/api/v1/manager/assign/:id) (req : put)
   =================================================== */
exports.assignManager = catchAsyncError(async (req, res, next) => {
  const manager = await Manager.findById(req.body.manager);
  const project = await Project.findById(req.params.id);
  if (!project) {
    return next(new ErrorHandler("Project not found", 400));
...
/* ===================================================
        Resign Client (/api/v1/client/resign/:id) (req : put)
   =================================================== */
exports.resignClient = catchAsyncError(async (req, res, next) => {
  ...
  if (client) {
    ...
    await Client.findByIdAndUpdate(client._id, data, {
      new: true,
      runValidators: true,
      useFindAndModify: false,
    });
  ...
  res.status(200).json({
    success: true,
    message: "Resign Client",
    project: newProject,
  });
});

/* ===================================================
        Assign Client (/api/v1/client/assign/:id) (req : put)
   =================================================== */
exports.assignClient = catchAsyncError(async (req, res, next) => {
  const client = await Client.findById(req.body.client);
  const project = await Project.findById(req.params.id);
  if (!project) {
    return next(new ErrorHandler("Project not found", 400));
...
/* ===================================================
        Create Attendence (/api/v1/create/attendence) (req : POST)
   =================================================== */
exports.createAttendence = catchAsyncError(async (req, res, next) => {
  const { data, time } = req.body;
  if (!data | !time) {
    return next(new ErrorHandler("Please enter data &  time", 400));
  }

  await Attendence.create({ data, time });
  res.status(200).json({
    success: true,
    message: "Successfully Attendence Created",
  });
});

/* ===================================================
        Get All Attendence (/api/v1/get/attendence) (req : Get)
   =================================================== */
exports.getAttendence = catchAsyncError(async (req, res, next) => {
